# Домашнее задание к занятию «Хранение в K8s» Соколов Тимофей
...
![zadanie2.5](images/zadanie2.5.png)

Почему PV не удалился сам после удаления PVC?
Когда я удалил PVC, PV не исчез сам. Он перешёл в статус Released. Это произошло потому, что у PV стояла политика Retain (сохранять). 
При такой политике Kubernetes не трогает PV после того, как PVC удалили. 
Это сделано специально, чтобы администратор сам решил, что делать с данными, удалить их, сохранить или куда-то перенести. 
Если бы была политика Delete, то PV удалился бы автоматически, но тут такого не произошло.

Почему файл остался на диске после удаления PV?
Я удалил PV командой kubectl delete pv local-pv. Сам объект PV пропал из Kubernetes, но файл на ноде (/mnt/data/pv-storage/data.txt) остался. 
Потому что PV типа hostPath — это просто ссылка на папку на компьютере. Удаление PV не удаляет реальные файлы с диска, это не входит в задачи Kubernetes. 
Очистка данных на ноде - это работа администратора. Так сделано для безопасности, чтобы случайно не потерять важные данные.
